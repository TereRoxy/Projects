...
void UserWindow::connectSignals()
{
	...
	QObject::connect(getUserEventsButton, &QPushButton::clicked, this, &UserWindow::showUserEventsWindow);
}
...
void UserWindow::handleSeeEventsByMonth()
{
	...
	if (eventIterator != nullptr)
		delete eventIterator;
	eventIterator = new MonthEventIterator(service, monthInt);
...
void UserWindow::showUserEventsWindow() {
	if (!userEventsWindow) {
		userEventsWindow = new UserEventsWindow;
	}

	std::vector<Event> events = service->getUserEvents();
	userEventsWindow->setEvents(events);
	userEventsWindow->show();
}
