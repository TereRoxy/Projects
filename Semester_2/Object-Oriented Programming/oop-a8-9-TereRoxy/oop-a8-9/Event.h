...
public:
	...
	friend istream& operator>>(istream& in, Event& event) {
		...
		in >> event.peopleNr >> event.title; 
		getline(in >> ws, event.description, '|');		
		in >> event.date >> event.time >> event.link;
		return in;
	}

	friend ostream& operator<<(ostream& out, const Event& event) {
		// Write each member variable to the output stream
		out << event.peopleNr << " " << event.title << " " << event.description << " | " << event.date << " " << event.time << " " << event.link << endl;
		return out;
	}
