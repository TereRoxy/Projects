...
class UndirectedGraph:
    ...
    def copy(self):
        ...
        """
        copy = UndirectedGraph(self.__vertices_nr)
    ...
    def addEdge(self, source: int, target: int, cost=None):
        ...
        self.__outbound_edges[target].append(source)  # Adding the reverse edge
        self.__inbound_edges[target].append(source)
        self.__inbound_edges[source].append(target)  # Adding the reverse edge
        self.__costs[(source, target)] = cost
        self.__costs[(target, source)] = cost  # Cost for reverse edge
        self.__edge_nr += 1
    ...
    def removeEdge(self, source: int, target: int):
        ...
        self.__outbound_edges[target].remove(source)  # Removing the reverse edge
        self.__inbound_edges[target].remove(source)
        self.__inbound_edges[source].remove(target)  # Removing the reverse edge
        self.__edge_nr -= 1
        del self.__costs[(source, target)]
        del self.__costs[(target, source)]  # Removing the cost for reverse edge
    ...
    def setCost(self, source: int, target: int, cost: int):
        ...
        """
        self.__costs[(source, target)] = cost
        self.__costs[(target, source)] = cost  # Setting the cost for reverse edge
